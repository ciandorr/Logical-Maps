...
- **$\mathrm{Possibility}_\vee$ disjunction closure.** ∀P,Q:tt. ($\mathrm{Possibility}_\vee$(P) ∧ $\mathrm{Possibility}_\vee$(Q)) → $\mathrm{Possibility}_\vee$(λp:t. P p ∨ Q p)

## Proof

Assume the two candidate predicates, which are themselves □-stable identities with ⊤. For R = λp.(Pp∨Qp), PSa follows by cases from the PSa clauses of P and Q. Neither disjunct of R⊥ can hold by their PSb clauses. For PSc, split P(p∨q)∨Q(p∨q), apply the respective PSc clause, and inject into Rp∨Rq. The same finite deduction is available necessarily: the candidate conditions are necessary, and necessitated logical theorems plus K transport the deduction under □. Thus the conjunction defining $\mathrm{Possibility}_\vee$(R) is identical to ⊤. Discharge and generalize P,Q.
