...
function uniqueJunctions(g){const seen=new Set();for(const j of g.nodes.filter(n=>n.kind==='junction')){const inputs=g.edges.filter(e=>e.to===j.id).map(e=>e.from).sort();const key=JSON.stringify(inputs);assert.ok(!seen.has(key),'One junction per displayed signed premise set');seen.add(key);assert.ok(inputs.length>=2);assert.equal(inputs.length,new Set(inputs).size);assert.ok(g.edges.some(e=>e.from===j.id));}}
try{
